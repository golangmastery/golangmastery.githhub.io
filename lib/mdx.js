...
export function getModuleFiles(courseSlug) {
  // Try the standard directory first
  let moduleDirectory = path.join(contentDirectory, 'courses', courseSlug);
  
  // If that doesn't exist, try with the -modules suffix (for backward compatibility)
  if (!fs.existsSync(moduleDirectory)) {
    moduleDirectory = path.join(contentDirectory, 'courses', `${courseSlug}-modules`);
  }
  
  // Check if module directory exists
  if (!fs.existsSync(moduleDirectory)) {
    console.log(`Module directory not found for ${courseSlug}`);
    return [];
  }
  ...
  const modules = files.map(file => {
    ...
    return {
      ...
      path: `${courseSlug}/${file.replace('.mdx', '')}`,
      title: data.title,
...
export function getContentFileBySlug(type, slug) {
  let contentTypeDirectory = path.join(contentDirectory, type);
  let filePath = path.join(contentTypeDirectory, `${slug}.mdx`);

  console.log(`Trying to access file: ${filePath}`);
  
  // Special handling for course modules
  if (type.startsWith('courses/') && !fs.existsSync(filePath)) {
    // If the file doesn't exist, try the alternate path with -modules suffix
    const parts = type.split('/');
    if (parts.length === 2) {
      const courseSlug = parts[1];
      const alternateType = `courses/${courseSlug}-modules`;
      const alternateDir = path.join(contentDirectory, alternateType);
      const alternatePath = path.join(alternateDir, `${slug}.mdx`);
...
export async function getAllContent(type) {
  ...
  const content = files.map(file => {
    ...
    return {
      frontmatter: data || {}, // Ensure frontmatter is always at least an empty object
      slug: file.replace('.mdx', ''),
    };
  });
