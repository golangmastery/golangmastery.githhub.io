...
UTF-8 is a character encoding where it assigns 1,112,064 characters a binary number that is from 1 byte (8 bits) to 4 bytes (32 bits) long.
...
It allows us to say this file is stored with the UTF-8 encoding, so the binary code must be interpreted with that in mind.
...
As we've seen with decimal, binary, and hexadecimal representation in Go, this example demonstrates the use of the `%q` annotation which is used for UTF-8 character encoding.
